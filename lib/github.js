...
api.getAllReposFromOrg = function getAllReposFromOrg(callback) {
  ...
  github.client.repos.getFromOrg(req, function(err, res) {
    if(err) {
      log.error({error: err.message}, 'Error - Github: getAllReposFromOrg');
      ...
      async.each(res, function(r, cb) {
        ...
        if(err) {
          log.error({error: err}, 'Error - Github: parsing getAllReposFromOrg');
          callback(err.message);
        }
        else {
          log.info({reposFromOrg: reposFromOrg.length}, 'Github: getAllReposFromOrg');
          log.trace({reposFromOrg: reposFromOrg}, 'Github: getAllReposFromOrg');
...
api.getAllPullRequestsFromOrg = function getAllPullRequestsFromOrg(callback) {
  ...
  async.each(repos, function(r, cb) {
    ...
    github.client.pullRequests.getAll(req, function(err, res) {
      if(err) {
        log.error({error: err.message}, 'Error - Github: getAllPullRequestsFromOrg');
        cb(err.message);
      }
      else {
    ...
    if(err) {
      log.error({error: err}, 'Error - Github: parsing getAllPullRequestsFromOrg');
      callback(err.message);
    }
    else {
      log.info({pullRequestsFromOrg: pullRequestsFromOrg.length}, 'Github: getAllPullRequestsFromOrg');
      log.trace({pullRequestsFromOrg: pullRequestsFromOrg}, 'Github: getAllPullRequestsFromOrg');
